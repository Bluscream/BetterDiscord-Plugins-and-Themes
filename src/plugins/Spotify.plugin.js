...
var Spotify = function () {
	'use strict';
	this.getName = function(){ return "Spotify Plugin"; }
	this.getDescription = function(){ return "Allows you to use some Spotify Features."; }
		...
		setInterval(function(){
			Spotify.client.getStatus(function (err, res) {
				if(err){
					err = err.toString();
					if(lastErr != err){
						lastErr = err; console.error(err);
					}else{return;}
				}
				if(lastURL != res.track.track_resource["location"]["og"]){
					BetterDiscordBot.constructor.bot.sendMessage( '159956363638734848', '<@134390773377400832> is now playing: '+ res.track.track_resource["location"]["og"]);
...
try{exports.Spotify = Spotify;}catch(e){console.warn('Spotify: Using old BetterDiscord version, not exporting functions.')}