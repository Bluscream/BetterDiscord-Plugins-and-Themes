...
BetterDiscordBot.prototype.start = function() {
	...
	$('span[data-reactid=".0.1.1.0.2.0.4.0:0.0"]').parent().before('<button id="bdlogbutton" type="button"><span style="background-image:url(\'https://cdn3.iconfinder.com/data/icons/file-format-2/512/log_.log_file_file_format_document_extension_format-128.png\');color:white;"></span></button>');
		...
		bot.on("message", function(message){
			...
			if(message.author.id != BetterAPI.getOwnID()){
				...
				if (BetterDiscordBotting.settings.enabled) {
					...
					switch(msg) {
					 case prefix+'help':
						bot.sendMessage(message.channel, '**BetterDiscordBot** by @Decorater and Bluscream.\n\nPrefix: '+prefix+'\nCommands:`info`,`time`');break;
					 case prefix+'info':
						bot.sendMessage(message.channel, "Credits:\n\n`Discord` by Hammer & Chisel\n`Discord.js` by hydrabolt\n`BetterDiscord` by Jiiks\n`BetterDiscord+` by Bluscream");break;
					 case prefix+'version':
						bot.sendMessage(message.channel, "Discord `0.0.287` **|** BetterDiscord `0.2.7` (JS `1.61`) **|** Discord.js `"+require("discord.js/package").version+"` **|** BetterDiscordBot `"+BetterDiscordBot.prototype.getVersion()+"`");break;
					 case prefix+'ping':
						if(BetterAPI.elemExists('.voice-connection-quality-fine')){
							bot.sendMessage(message.channel, "Voice connection quality is fine.");
						}
						break;
					 case prefix+'time':
						bot.sendMessage(message.channel, 'My local time is `'+now+'`');break;
					 case prefix+'finger':
						bot.sendMessage(message.channel, 'echo "Ohh, yeah i like that :3"');break;
					 default:
						 break;
					}
				}
				if(BetterDiscordBotting.settings.gifavatar){
					...
					if(sn && lastBotUser != message.author.id){
						...
						bot.sendMessage(message.channel, '<@'+message.author.id+'> Uploading GIF avatars was a bug that is fixed since dec 2015. That means you can not set them anymore.');
