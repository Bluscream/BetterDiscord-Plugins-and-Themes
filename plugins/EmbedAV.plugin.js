//META{"name":"EmbedAV"}*//
function EmbedAV(){this.initiated=false;this.optionsDefault={embedAudio:true,embedVideo:true,embedImage:true,embedCode:true,embedLoop:false,embedAutoPlay:false,volume:0.5};var options=this.optionsDefault,filterAudio='\
...
try{accessory.find(tagName).get(0).volume=options.volume;}catch(e){}});messages.scrollTop+=messages.scrollHeight-mHeight;};this.apply=function(preventAutoPlay,seeLastMessage,log=true){var links,embedAutoPlay;if(seeLastMessage){links=$('.message:eq(-1) .markup a:not(.EmbedAV-processed),.message:eq(-1) .attachment a:not(.EmbedAV-processed)');}
else{links=$('.message .markup a:not(.EmbedAV-processed),.message .attachment a:not(.EmbedAV-processed)');}
if(!this.initiated){this.init();this.initiated=true;}
embedAutoPlay=options.embedAutoPlay;if(preventAutoPlay){console.log("[BetterDiscord] EmbedAV: Preventing Autoplay");options.embedAutoPlay=false;}
...
$tagSettingsAutoPlay.find(':checkbox').attr('checked','checked');$tagSettingsVolume.find('[type="range"]').attr('value',options.volume);return'<ul>'
+'<li>'
