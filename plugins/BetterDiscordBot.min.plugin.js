...
function BetterDiscordBot() { 'use strict'; }
...
/* ==================================================================================================================================================================*/
		bot.on("message", function(message){
			// if(!message.channel){ var channel = ''+BetterAPI.getCurrentTextChannelID(); }
			...
			if(message.author.id != BetterAPI.getOwnID()){
				...
				if (BetterDiscordBotting.settings.enabled) {
					...
					switch(msg) {
					 case prefix+'help':
						sendMSG = '**BetterDiscordBot** by @Decorater and Bluscream.\n\nPrefix: '+prefix+'\nCommands:`info`,`time`';break;
					 case prefix+'info':
						sendMSG = "Credits:\n\n`Discord` by Hammer & Chisel\n`Discord.js` by hydrabolt\n`BetterDiscord` by Jiiks\n`BetterDiscord+` by Bluscream";break;
					 case prefix+'version':
						sendMSG = "Discord `0.0.287` **|** BetterDiscord `0.2.7` (JS `1.61`) **|** Discord.js `"+require("discord.js/package").version+"` **|** BetterDiscordBot `"+BetterDiscordBot.prototype.getVersion()+"`";break;
					 case prefix+'ping':
						if(BetterAPI.elemExists('.voice-connection-quality-fine')){
							sendMSG = "Voice connection quality is fine.";break;
						}
						break;
					 case prefix+'time':
						sendMSG = 'My local time is `'+now+'`';break;
					 case prefix+'finger':
						sendMSG = 'echo "Ohh, yeah i like that :3"';break;
					 default:
						 break;
					}
				}
		...
		bot.on("channelCreated", function(channel){
			if(BetterDiscordLogging.events.channelCreated){if(debugging){BetterDiscordBot.debug('channelCreated', arguments);}
				try{BetterDiscordBot.log('info', 'channelCreated', '<a href="https://discordapp.com/channels/'+channel.server.id+'/">'+channel.server.name+'</a>', '', 'Channel <a href="https://discordapp.com/channels/'+channel.server.id+'/'+channel.id+'">#'+channel.name+'</a> was created'+'.');
				}catch(e){}
			}
		});
		bot.on("channelUpdated", function(oldchannel, channel){
			if(BetterDiscordLogging.events.channelUpdated){if(debugging){BetterDiscordBot.debug('channelUpdated', arguments);}
				try{BetterDiscordBot.log('info', 'channelUpdated', '<a href="https://discordapp.com/channels/'+channel.server.id+'/">'+channel.server.name+'</a>', '', 'Channel '+wN(oldchannel.name)+' was edited to <a href="https://discordapp.com/channels/'+channel.server.id+'/'+channel.id+'">#'+channel.name+'</a>.');
				}catch(e){}
			}
		});
		bot.on("channelDeleted", function(channel){
			if(BetterDiscordLogging.events.channelDeleted){if(debugging){BetterDiscordBot.debug('channelDeleted', arguments);}
				try{BetterDiscordBot.log('info', 'channelDeleted', '<a href="https://discordapp.com/channels/'+channel.server.id+'/">'+channel.server.name+'</a>', '', 'Channel '+wN(channel.name)+' was deleted'+'.');
				}catch(e){}
			}
		});
